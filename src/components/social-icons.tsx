...
function IconLink({
  ...
  return (
    <a
      ...
      rel="noreferrer"
      aria-label={label}
      className="inline-flex h-7 w-7 items-center justify-center text-text transition-opacity hover:opacity-70"
    >
      {children}
    </a>
  );
  ...
  const items = [
    profile.instagram_url && (
      <IconLink key="ig" href={profile.instagram_url} label="Instagram">
        <svg viewBox="0 0 24 24" className="h-full w-full" fill="none" stroke="currentColor" strokeWidth="1.7">
